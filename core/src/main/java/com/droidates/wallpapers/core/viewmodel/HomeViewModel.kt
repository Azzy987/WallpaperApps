...
import com.droidates.wallpapers.core.model.Banner
...
import com.droidates.wallpapers.core.utils.WallpaperDiskCache
...
class HomeViewModel @Inject constructor(
    ...
    /** True while the first load is running; blocks re-entrant callers. */
        ...
        viewModelScope.launch(Dispatchers.IO) {
            _wallpapers.collect { wallpapers ->
                ...
                if (wallpapers.isNotEmpty()
                    ...
                    throw kotlinx.coroutines.CancellationException("Cache saved successfully, stopping collector")
                }
            }
        }
    ...
    fun loadInitialDataIfNeeded() {
        ...
        // The empty-list check must not re-open the gate while the first load is
        // still in flight — the caller is a LaunchedEffect that re-fires on tab
        // state changes, which previously caused two identical Firestore reads.
        if (isInitialLoadInFlight) return
        ...
        viewModelScope.launch(Dispatchers.IO) {
          try {
            ...
            try {
                // joinAll so the in-flight flag below is not cleared before these finish
                listOf(
            ...
            // Cleared on every exit path (cache-hit early return, error, success)
    ...
    fun ensurePaywallBannersLoaded() {
        if (_paywallBanners.value.isNotEmpty()) return
        viewModelScope.launch {
            loadPaywallBanners()
            if (_paywallBanners.value.isEmpty()) {
                loadPaywallBackdropFallback()
            }
        }
    }

    private suspend fun loadPaywallBackdropFallback() {
        try {
            withContext(Dispatchers.IO) {
                val documents = firestore.collection(AppConfig.COLLECTION_HOME)
                    .limit(12)
                    .get()
                    .await()
                val bannerList = documents.mapNotNull { doc ->
                    val url = doc.getString("thumbnail")?.takeIf { it.isNotBlank() }
                        ?: doc.getString("imageUrl")?.takeIf { it.isNotBlank() }
                        ?: return@mapNotNull null
                    Banner(
                        wallpaperId = doc.id,
                        bannerUrl = url,
                        bannerName = doc.getString("wallpaperName") ?: "",
                        exclusive = doc.getBoolean("exclusive") == true
                    )
                }
            ...
            Log.e(TAG, "Error loading paywall backdrop fallback", e)
        }
    }
    ...
    private suspend fun loadBanners() {
        ...
        try {
            withContext(Dispatchers.IO) {
                if (VERBOSE_LOGGING) {
                    Log.d(TAG, "Attempting to fetch banners from: Banners/${AppConfig.DOCUMENT_BANNERS_APP}/${AppConfig.COLLECTION_BANNERS_SUB}")
                }
                ...
                if (VERBOSE_LOGGING) {
                    Log.d(TAG, "Loaded ${bannerList.size} banners from Banners/${AppConfig.DOCUMENT_BANNERS_APP}/${AppConfig.COLLECTION_BANNERS_SUB}")
                }
            }
        } catch (e: Exception) {
            if (VERBOSE_LOGGING) {
