...
import com.droidates.wallpapers.core.viewmodel.CategoryViewModel
...
import com.droidates.wallpapers.core.utils.ScrollUtils
...
import com.droidates.wallpapers.core.ui.components.SeriesFilterDialog
...
fun CategoryScreen(
    ...
    // Restore the saved position once the grid actually has items.
    //
    // rememberLazyGridState applies its initial index only on first composition. When
    // returning from a wallpaper the grid is briefly empty (data re-emits a frame later),
    // so that initial index is clamped to 0 and the restore is lost. Re-applying it after
    // the items arrive is what makes "go back" land where the user left off.
    val restoredForCategory = remember { mutableStateOf<String?>(null) }
    LaunchedEffect(categoryName, categoryWallpapers.size) {
        if (categoryWallpapers.isEmpty()) return@LaunchedEffect
        if (restoredForCategory.value == categoryName) return@LaunchedEffect

        val (index, offset) = CategoryScreenScrollStates.scrollPositions[categoryName] ?: (0 to 0)
        restoredForCategory.value = categoryName
        if (index > 0 || offset > 0) {
            // Only scroll to an item that exists, otherwise the call is a no-op.
            if (index < categoryWallpapers.size) {
    ...
    //
    // This used to pass `derivedStateOf { … }` objects as LaunchedEffect keys. Those are
    // stable references, so the keys never changed, the effect ran exactly once on entry
    // (when the grid was still at 0) and never again — the position was always stored as
    // (0, 0) and returning from a wallpaper jumped back to the top.
    //
    // snapshotFlow observes the *values* instead, emitting on every scroll change.
    LaunchedEffect(categoryName) {
        snapshotFlow { scrollState.firstVisibleItemIndex to scrollState.firstVisibleItemScrollOffset }
            .collect { (index, offset) ->
                // Don't persist while the list is still empty, or a pre-data frame would
                // overwrite a good saved position with (0, 0).
                if (categoryWallpapers.isNotEmpty()) {
                    CategoryScreenScrollStates.scrollPositions[categoryName] = index to offset
                }
            }
    }
    ...
    val shouldLoadMore = PaginationUtils.shouldLoadMore(
        debounceKey = "CategoryScreen:$categoryName:${selectedSubcategory ?: "all"}",
        ...
        Column(
            ...
            Box(
                ...
                if (validSubcategories.isNotEmpty() && !categoryName.equals(
                    ...
                    // Sync only after swipe settles to avoid per-frame filter churn.
                    LaunchedEffect(pagerState, pageCount, allSubcategoryOptions) {
                        snapshotFlow { pagerState.settledPage % pageCount }
                    ...
                    HorizontalPager(
                        ...
                        val pageShouldLoadMore = PaginationUtils.shouldLoadMore(
                            debounceKey = "CategoryScreen:$categoryName:${pageSubcategory ?: "all"}",
                ...
                if (categoryName.equals(
                    ...
                    // Sync only when pager settles to keep swiping smooth.
                    LaunchedEffect(seriesPagerState, pageCount, allSeriesOptions) {
                        snapshotFlow { seriesPagerState.settledPage % pageCount }
                            .distinctUntilChanged()
                            .collect { actualPage ->
                                val newSeries =
                    ...
                    HorizontalPager(
                        ...
                        val pageShouldLoadMore = PaginationUtils.shouldLoadMore(
                            debounceKey = "CategoryScreen:$categoryName:series:${pageSeries ?: "all"}",
                            gridState = pageGridState,
                            itemCount = pageWallpapers.size,
                            isLoading = pageIsLoading,
                            hasReachedEnd = pageHasReachedEnd
    ...
    AlertDialog(
        ...
        text = {
            Column(
                ...
                Text(stringResource(R.string.depth_effect_note, AppConfig.DEVICE_FAMILY_NAME))
