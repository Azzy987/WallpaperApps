...
object Material3Motion {
    ...
    fun contentFadeSpec(reducedMotion: Boolean) = tween<Float>(
        durationMillis = if (reducedMotion) 80 else DURATION_SHORT,
        easing = emphasizedDecelerate
    )

    fun enterContainerSpec(reducedMotion: Boolean) = tween<Float>(
        durationMillis = if (reducedMotion) 80 else 260,
        easing = emphasizedDecelerate
    )

    fun actionPressSpec(reducedMotion: Boolean) = if (reducedMotion) {
